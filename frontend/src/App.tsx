...
const App: React.FC = () => {
  ...
  const handleFetchFileContent = async (path: string, name: string) => {
    ...
    try {
      const response = await fetch('http://127.0.0.1:8000/file-content', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const FileExplorerItem = ({ node, depth = 0, parentPath = "" }: { node: FileNode, depth?: number, parentPath?: string }) => {
        ...
        <div
          onClick={() => {
            if (node.type === 'directory') {
              ...
              handleFetchFileContent(fullPath, node.name);
            }
          }}
          ...
          <div className="animate-in fade-in slide-in-from-left-2 duration-200">
            {hasChildren ? (
              node.children!.map((child, idx) => (
                <FileExplorerItem key={idx} node={child} depth={depth + 1} parentPath={fullPath} />
              ))
            ) : (
              <div
